...
from OCP.ShapeFix import ShapeFix_Edge, ShapeFix_Shape
...
def solid_centroid(shape: TopoDS_Shape) -> np.ndarray:
    """Volume centroid of a solid — where to look to find the body in the part."""
    props = GProp_GProps()
    BRepGProp.VolumeProperties_s(shape, props)
...
def geometry_faults(shape: TopoDS_Shape) -> int:
    """How many exact B-rep faults OCCT's argument analyzer finds in ``shape``.

    ``BRepCheck_Analyzer`` is not enough on its own here, and the case that
    shows why is `SpiralTest.step`'s 4.17 mm³ floating island: it passes
    ``BRepCheck_Analyzer``, is closed, and is confirmed **not**
    self-intersecting — while carrying 29 ``BOPAlgo_InvalidCurveOnSurface``
    faults, pcurves that wander up to 2.118e-02 mm from their own 3D curves.
    It passes because that deviation is exactly the tolerance those edges
    record; on a body whose faces are ~0.05 mm² the tolerance is the size of
    the feature. Nothing can tessellate it consistently, which is how it
    surfaced — 9 non-manifold edges and 4 apparent self-crossings in 135
    triangles.

    Only ever asked of small solids (:data:`latticegen2.pipeline.DEGENERATE_ISLAND_MAX_VOLUME`),
    because a whole-solid analysis of a 45,825-face lattice is the class of
    OCCT call this project has already measured at 14.5 GB.
    """
...
def faults_after_step_roundtrip(shape: TopoDS_Shape, path: str) -> int:
    """Exact faults in ``shape`` after it has been written to STEP and read back.

    **The quantity that matters is the one in the file the user receives**, and
    it is not the one in memory. Measured on `SpiralTest.step`'s 4.17 mm³
    island: **0** faults as the pipeline builds it, **29**
    ``InvalidCurveOnSurface`` after export — pcurves regenerated by the STEP
    round trip that wander up to 2.118e-02 mm from their own 3D curves. That is
    what makes the body impossible to tessellate consistently, and a check run
    before export cannot see it by construction.

    One body at a time, so this costs a small file rather than a re-read of the
    whole output — the round-trip re-import of a 2 GB result was removed from
    this pipeline precisely because it cost 22 minutes (§9).
    """
    write_step(shape, path, "roundtrip-probe")
    return geometry_faults(read_step(path))


def try_repair_geometry(shape: TopoDS_Shape) -> TopoDS_Shape | None:
    """``ShapeFix_Shape`` applied to ``shape``, or ``None`` if it did not help.

    "Too degenerate to be saved by other methods" is a claim this makes rather
    than assumes: the repair is attempted, and the result is accepted only when
    it clears every fault, comes back a single valid solid, and preserves the
    volume. Measured on the island above, both of OCCT's candidates fail —
    ``ShapeFix_Shape`` returns it bit-identical (same volume and area, still 9
    bad mesh edges) and ``BRepLib::SameParameter`` makes it
    ``BRepCheck_Analyzer``-**invalid** without fixing the tessellation. So the
    drop below is a last resort with the alternatives measured, not assumed.
    """
    before = volume(shape)
    fixer = ShapeFix_Shape(shape)
    fixer.Perform()
    fixed = fixer.Shape()
    solids_out = solids(fixed)
    if len(solids_out) != 1:
        return None
    repaired = solids_out[0]
    if geometry_faults(repaired):
        return None
    if not is_valid(repaired):
        return None
    after = volume(repaired)
    if abs(after - before) > 1e-6 * max(abs(before), 1.0):
        return None
    return repaired
